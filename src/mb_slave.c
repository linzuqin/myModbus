...
mb_err_code_t mb_slave_check(mb_dev_t *mb_dev, mb_func_code_t func_code, uint16_t start_addr, uint16_t quantity)
{
    ...
    if (start_addr < dev_start_addr || start_addr >= (dev_start_addr + buf_size))
    {
        return Illegal_Data_Address;
    }

    relative_addr = start_addr - dev_start_addr;

    if (quantity < 1 || quantity > max_size)
    {
        return Illegal_Data_Value;
    }

    if ((relative_addr + quantity) > buf_size)
    {
        return Illegal_Data_Address;
    }
    ...
    {
        case MB_FUNC_WRITE_MULTIPLE_COILS:
            if (quantity > 1968) // ai搜索的Modbus协议限制
                return Illegal_Data_Value;
            break;
            
        case MB_FUNC_WRITE_MULTIPLE_REGISTERS:
            if (quantity > 123) // Modbus协议限制
                return Illegal_Data_Value;
            break;
            
        case MB_FUNC_READ_COILS:
        case MB_FUNC_READ_DISCRETE:
            if (quantity > 2000) // Modbus协议限制
                return Illegal_Data_Value;
            break;
            
        case MB_FUNC_READ_HOLDING:
        case MB_FUNC_READ_INPUT:
            if (quantity > 125) // Modbus协议限制
                return Illegal_Data_Value;
            break;
...
static mb_err_t mb_s_build_response(mb_dev_t *mb_dev, mb_func_code_t func_code, uint16_t start_addr, uint16_t reg_count, uint8_t *response, uint16_t response_size)
{
    ...
    switch (func_code)
    {
        case MB_FUNC_READ_COILS:    // 读线圈寄存器
        case MB_FUNC_READ_DISCRETE: // 读离散输入寄存器
        {
            ...
            else
            {
                ...
                for (uint16_t i = 0; i < reg_count; i++)
                {
                    ...
                    {
                        status = mb_dev->mb_coil_reg[start_addr + i - mb_dev->coil_start_addr];
                    }
                    else
                    {
                        status = mb_dev->mb_disc_reg[start_addr + i - mb_dev->disc_start_addr];
                    }
        ...
        case MB_FUNC_READ_INPUT:   // 输入寄存器
        {
            ...
            else
            {
                ...
                for (uint16_t i = 0; i < reg_count; i++)
                {
                    ...
                    {
                        reg_value = mb_dev->mb_hold_reg[start_addr + i - mb_dev->hold_start_addr];
                    }
                    else
                    {
                        reg_value = mb_dev->mb_input_reg[start_addr + i - mb_dev->input_start_addr];
                    }
        ...
        case MB_FUNC_WRITE_SINGLE_COIL: // 线圈写入
        {
            ...
            {
                if (mb_dev->mb_coil_reg[start_addr - mb_dev->coil_start_addr])
                {
                    response[4] = 0xFF;
                    response[5] = 0x00;
                }
        ...
        case MB_FUNC_WRITE_SINGLE_REGISTER: // 单个保持寄存器写入
        {
            ...
            {
                response[4] = (uint8_t)(mb_dev->mb_hold_reg[start_addr - mb_dev->hold_start_addr] >> 8);   // 值高位
                response[5] = (uint8_t)(mb_dev->mb_hold_reg[start_addr - mb_dev->hold_start_addr] & 0xFF); // 值低位
            }
            break;
        }
...
static mb_err_t mb_s_parse(mb_dev_t *mb_dev)
{
    ...
    mb_dev->index_func_code = function_code;
    ...
    {
        mb_dev->tx_size = 5;
        tx_buf[0] = mb_dev->addr;
        tx_buf[1] = function_code | 0x80; // 设置异常标志
        tx_buf[2] = check_result;         // 异常码

        uint16_t crc = usMBCRC16(tx_buf, 3);
        tx_buf[3] = (uint8_t)(crc & 0xFF);
        tx_buf[4] = (uint8_t)((crc >> 8) & 0xFF);
    ...
    switch (function_code)
    {
        ...
        case MB_FUNC_WRITE_SINGLE_COIL: // 写单个线圈
            reg_count = 1;
            parse_result = mb_s_coil_parse(mb_dev, start_addr - mb_dev->coil_start_addr , mb_dev->rx_buffer[MB_VALUEH_BIT] << 8 | mb_dev->rx_buffer[MB_VALUEL_BIT]);
            break;

        case MB_FUNC_WRITE_SINGLE_REGISTER: // 写单个保持寄存器
            reg_count = 1;
            parse_result = mb_s_hold_parse(mb_dev, start_addr - mb_dev->hold_start_addr, mb_dev->rx_buffer[MB_VALUEH_BIT] << 8 | mb_dev->rx_buffer[MB_VALUEL_BIT]);
            break;
        ...
        case MB_FUNC_WRITE_MULTIPLE_COILS: // 写多个线圈
            ...
            {
                parse_result = mb_s_coils_parse(mb_dev, start_addr - mb_dev->coil_start_addr, &mb_dev->rx_buffer[MB_BYTE_COUNT_BIT + 1], reg_count);
            }
            break;
        ...
        case MB_FUNC_WRITE_MULTIPLE_REGISTERS: // 写多个保持寄存器
            ...
            {
                parse_result = mb_s_holds_parse(mb_dev, start_addr - mb_dev->hold_start_addr, &mb_dev->rx_buffer[MB_BYTE_COUNT_BIT + 1], reg_count);
            }
            break;
    ...
    {
        mb_dev->tx_size = size;
        tx_buf[0] = mb_dev->addr;  // 设备地址
        tx_buf[1] = function_code; // 功能码

        parse_result = mb_s_build_response(mb_dev, function_code, start_addr, reg_count, tx_buf, size);
...
void mb_s_poll(void)
{
    ...
    {
        switch(mb_slave_devs[i].mb_status)
        {
            case MB_IDLE:
            {
                if(mb_get_tick() - mb_slave_devs[i].last_poll_time > mb_slave_devs[i].poll_interval)
                {
                    //超过预设的轮询时间 就判定为设备离线
                    mb_slave_devs[i].mb_status = MB_OFFLINE;
                }
                else if(mb_slave_devs[i].rx_size > 0)
                {
                    mb_slave_devs[i].mb_status = MB_PARSE;
                }
                break;
            }
            ...
            case MB_PARSE:
            {
                if(mb_s_parse(&mb_slave_devs[i]) == MB_OK)
                {
                    mb_slave_devs[i].devOnline = 1;//恢复为设备在线
                    mb_slave_devs[i].mb_status = MB_RESP;
                    mb_slave_devs[i].last_poll_time = mb_get_tick();

                }
                else
                {
                    mb_slave_devs[i].mb_status = MB_ERROR;
                }
                break;
            }
            ...
            case MB_RESP:
            {
                if(mb_slave_devs[i].send_callback != NULL)
                {
                    mb_slave_devs[i].send_callback(mb_slave_devs[i].tx_buffer , mb_slave_devs[i].tx_size);
                }
                mb_clean(&mb_slave_devs[i]);
                break;
            }
            ...
            {
                //设备离线处理
                mb_slave_devs[i].devOnline = 0;

                //自定义离线处理
                mb_slave_devs[i].last_poll_time = mb_get_tick();
                mb_slave_devs[i].mb_status = MB_IDLE;
                break;
            }
